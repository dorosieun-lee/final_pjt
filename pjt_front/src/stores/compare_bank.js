...
import { userCheckStore } from './usercheck'

export const compareBankStore = defineStore('comparebank', () => {
  ...
  const Deposit_list = function () {
    axios({
      method: 'get',
      url: `${API_URL}/api/v1/bank-products/deposit-products/`,
      headers: {
  ...
  const Saving_list = function () {
    axios({
      method: 'get',
      url: `${API_URL}/api/v1/bank-products/saving-products/`,
      headers: {
        Authorization: `Token ${store.token}`
      }
    })
  ...
  const Deposit_option = function () {
    axios({
      method: 'get',
      url: `${API_URL}/api/v1/bank-products/deposit-products-options/${fin_prdt_cd}`,
      headers: {
        Authorization: `Token ${store.token}`
      }
    })
  ...
  const Saving_option = function () {
    axios({
      method: 'get',
      url: `${API_URL}/api/v1/bank-products/saving-products-options/${fin_prdt_cd}`,
      headers: {
        Authorization: `Token ${store.token}`
      }
    })
  ...
  return {
    API_URL, Deposit_list, depositList, savingList, Saving_list,
    bankLink, searchBankLink, Deposit_option, Saving_option, depositOption, savingOption
  }
}, { persist: true })