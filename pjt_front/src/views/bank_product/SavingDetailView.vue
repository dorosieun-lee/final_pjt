...
onMounted(() => {
  ...
  axios({
    method: 'get',
    url: `${store.API_URL}/api/v1/bank-products/saving-detail/${fin_prdt_cd}`,
    headers: {
...
const check_likes_user = function () {
  axios({
    method: 'get',
    url: `${store.API_URL}/api/v1/bank-products/${route.params.id}/likes-check-saving/`,
    headers: {
        Authorization: `Token ${user_store.token}`
      }
  })
    .then((res) => {
      if (res.data.user) {
        buttonText.value = '관심없어요'
      }
      else {
        buttonText.value = '관심있어요'
      }
    })
    .catch((err) => {
      console.log(err)
...
const onClick = function () {
  axios({
    method: 'post',
    url: `${store.API_URL}/api/v1/bank-products/${route.params.id}/likes-saving/`,
    headers: {
      Authorization: `Token ${user_store.token}`
    }
  })
  ...
  <div class="container mt-5">
    ...
    <div class="btn-toolbar" role="toolbar" aria-label="Toolbar with button groups">
      <div class="btn-group me-2" role="group" aria-label="First group">
        <button @click="onClick" class="btn" :class="{'btn-primary': buttonText === '관심있어요', 'btn-danger': buttonText === '관심없어요'}">{{ buttonText }}</button>
      </div>
