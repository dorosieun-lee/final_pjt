...
const search = () => {
    isSearch.value = true
    boards.value = boardStore.LboardAll.filter((item) => {
        // console.log(item)
        ...
        if (target.value === 'local') {
            if (item.address.includes(keyword.value)) {
                return item
            }
        }
    })
...
<template>
    <div class="container">
        <h2><img src="/assets/local_marker.png" alt="" style="width: 2.3rem;"> 지역 커뮤니티</h2>
        <p class="my-0">게시판 주제에 맞는 게시글 작성을 부탁드립니다.</p>
        <p>게시글 작성 전, 개인정보 수정에서 지역 설정하기!</p>
        <RouterLink :to="{name: 'localCreate'}" class="btn btn-primary my-1">게시글 작성하기</RouterLink>
        ...
        <ul class="list-group list-group-flush text-start" v-if=boardStore.LboardAll>
            <li v-if="!isSearch" v-for="board in boardStore.LboardAll" :key="board.id" class="list-group-item">
                <div class="row">
                    <p class="col-5 my-auto">제목: {{ board.title }}</p>
                    <p class="col-3 my-auto">작성일: {{ formatDate(board.created_at) }}</p>
                    <p class="col-2 my-auto">지역: {{ board.address ? board.address.split(' ')[0] : "Unknown"}}</p>
