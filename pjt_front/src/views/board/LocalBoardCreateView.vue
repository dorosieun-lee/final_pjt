...
const createBoard = function () {
    axios({
        method: 'post',
        url: `${userStore.API_URL}/api/v1/boards/local/`,
        data: {
            title: title.value,
...
const updateBoard = function () {
    axios({
        method: 'put',
        url: `${userStore.API_URL}/api/v1/boards/local/${route.params.board_pk}/`,
        data: {
            title: title.value,
            content: content.value,
        },
...
if (route.name === 'localUpdate') {
    isCreate.value = false
    title.value = boardStore.LboardDetail.title
    content.value = boardStore.LboardDetail.content
} 
</script>
