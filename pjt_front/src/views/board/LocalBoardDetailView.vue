...
const getBoardDetail = function (boardName, boardPk) {
    ...
    .then((res) => {
        ...
        comments.value = res.data.local_comment
        boardStore.LboardDetail = res.data
    })
    .catch((err) => {
        console.log(err)
    })
...
const updateBoard = function () {
    router.push({name: 'localUpdate', params: {board_pk: boardPk}})
}

const deleteBoard = function () {
    axios({
        method: 'delete',
        url: `${userStore.API_URL}/api/v1/boards/local/${boardPk}/`,
        headers: {
            Authorization: `Token ${userStore.token}`
        }
    })
...
<template>
    <div class="container">
        ...
        <div v-if="board" class="my-4">
            ...
            <p>지역: {{ board.address ? board.address : "Unknown"}}</p>
        ...
        <hr>
        <RouterLink :to="{name: 'local'}" style="text-decoration: none;">전체 게시글 보기</RouterLink>
        <hr>
        <CreateComment board-name="local" @create-comment="getBoardDetail(boardName, boardPk)"/>
